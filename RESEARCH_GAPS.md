...
Snapshot: 2026-07-27 (updated after Commit W docs sync, post T/U/V audit remediation). Baseline: `main` at `a4e135b`, spanning commits `9b2a025` … `a4e135b`.
...
**Current tool speaks:** VMProtect (all 5 version buckets) plus a class-level "some protector" gate and 9 vendor byte-table matchers (Themida/WinLicense, Code Virtualizer, Enigma, Obsidium, Armadillo, ASPack, Denuvo detect+refuse, BattlEye, Vanguard), plus 5 compressor families (UPX/MPRESS/Petite/PECompact/Upack) short-circuited as "packed, not virtualised". VMP version buckets — 1.x, 2.x, 3.0-3.4, 3.5.0-3.5.1, 3.6-3.10.5 — scored from `.vmp0`/`.vmp1` section presence, VMP-1-style `pushad; mov esi, imm32; lea edi, [esi+disp]` entry stub, VMP-2/3-style `push imm32; call/jmp rel32` entry stub landing in a `.vmpN` section, the literal `"VMProtect"` string, `ZwProtectVirtualMemory` (3.x marker), and an RWX entry-section check. **35/35** VMP-semantic handler shapes in the taxonomy are now accounted for (~30 reachable through `classify()`; `Str`/`Vexec`/`Vunk` are documented sentinel-only — see §3.2). A junk-code stripper (peephole Groups A-G + fixed-point iteration) runs before semantic matching. A register-role canonicaliser (`register_roles.rs` + cross-handler consistency gate) votes VIP/VSP/VKEY per binary. Dispatch table located by a fallback pointer-run scan over `.text`/`.rdata`/`.vmp*`/`.kbB*`, plus a structural three-instruction dispatcher fingerprint scan for renamed-section VMP (BattlEye, EAC). Operand decryption dispatches on `CryptoScheme` per VMP version (`Vmp2Rolling`, `Vmp3PerHandler`, replacing the old universal `crc*31+val` placeholder, which is retained only as a named fallback variant). ALU chains recognise homogeneous NOR/NAND runs and stamp `ADD` / `SUB` / `NOT` / `AND` on the last instruction of the run. A `--export-analysis` unified JSON export and two sample-validation harnesses (`--features real-samples`, `--features synthetic-samples`) exist.

**Current tool still misses (headline) — carried over from before Commits E-V, now narrowed:**
- Real-sample validation is still open: the synthetic-sample harness (Commit S) proves the pipeline agrees with itself structurally, but no live VMP-protected binary has been run end-to-end since the audit. `tests/fixtures/vmp*/` remain empty pending user-provided samples.
- Import-table restoration is not implemented (RESEARCH_GAPS §7 item #10, XL effort — needs an emulator).
- MBA / opaque-predicate simplification at the Souper/QSynth/msynth level is not implemented — the junk stripper (Groups A-G) handles peephole-level dead code and constant folding, not general mixed-boolean-arithmetic identities.
- VMP 3.7+ merged-handler support is not implemented. `register_roles.rs`'s per-binary voter (Commit K) is the necessary foundation (canonical VIP/VSP/VKEY assignment), but a cross-handler DFA to split merged handlers back into logical ops has not been built.
- Non-VMProtect *semantic* handler classification (Themida ISA, Code Virtualizer opcodes, Denuvo white-box) is still `Unknown` beyond family/version detection — §3.2/§3.3 below describe why the current VMP-specific matchers (De Morgan ALU, fixed register conventions) do not generalise.
- No control-flow-flattening removal, no self-modifying-code / TLS-callback bootstrap detection, no anti-debug catalogue.

**Next-biggest wins by effort (remaining after Commits E-V):**
- Real-sample corpus: 3-5 VMP-protected hello-world builds (VMProtect Free/Trial) dropped into `tests/fixtures/vmp*/` turn the existing `--features real-samples` harness (Commit N) from "0 samples, always skips" into actual pass/fail feedback, and lets Days 6-7 semantic validation (crypto-chain correctness, ALU-chain correctness) finally proceed (M effort once samples exist).
- Import-table restoration (§7 item #10) — end-to-end devirtualised output with real call targets instead of raw pointers (XL, needs an emulator; vmpfix/VMPImportFixer as permissive-license reference).
- MBA / opaque-predicate solver at Souper/QSynth/msynth level (§3.3) — collapses the remaining junk arithmetic the peephole stripper can't fold (XL).
- Cross-handler DFA on top of the register-role voter (§Q7 in roadmap) to support VMP 3.7+ merged handlers — the single biggest remaining VMP-version gap (XL).
...
| **Themida / WinLicense** | 1.x–1.9 (fixed EP prelude) | ✅ | ⛔ | ⛔ | Section-name allowlist + v1.x compressed/uncompressed/DLL entry-stub byte tables + `"Themida"`/`"WinLicense"`/`"Oreans"` string markers, all in `src/protector_matchers.rs` (Commit F). No sub-version discrimination or handler semantics — family-level only. |
| Themida | 2.x, 3.x (post-2020, per-build randomised section names) | 🟡 | ⛔ | ⛔ | Structural signals (RSP = VSP, 2-byte opcodes, nested VMs) not modelled; only the fixed v1.x-era stubs/strings fire, so post-2020 randomised builds still depend on incidental string-marker survival. |
...
| **Code Virtualizer** (Oreans standalone) | 1.3.x – 2.2.x | ✅ | ⛔ | ⛔ | Dispatcher fingerprint `AC 0F B6 C0 FF 24 87` (`lodsb; movzx eax,al; jmp [edi+eax*4]`) scanned in `protector_matchers.rs` (Commit F). |
| Code Virtualizer | Stealth mode (VM inlined) | ⛔ | ⛔ | ⛔ | Requires dispatcher-shape scan inside `.text` specifically; current scan covers the default last-section placement, not the inlined stealth variant. |
| **Enigma Protector** | 1.x – 3.79 | ✅ | ⛔ | ⛔ | `.enigma0`–`.enigma3` section names + `"Enigma protector"`/`"ENIGMA"` string markers + entry-stub prelude, all in `protector_matchers.rs` (Commit F). |
...
| **Obsidium** | 1.3.x – 1.7.x | ✅ | ⛔ | ⛔ | Short-jump anti-disasm entry-stub pattern (`EB 03 ?? ?? ?? E8 …`) implemented in `protector_matchers.rs` (Commit F). |
| **Armadillo / SoftwarePassport** | 3.x – 9.x (legacy) | ✅ | ⛔ | ⛔ | `.text1`/`.adata`/`.pdata` section names + entry-stub prelude, implemented in `protector_matchers.rs` (Commit F). |
| **ASPack / ASProtect** | current | ✅ | ⛔ | ⛔ | `.aspack`/`ASPack`/`.adata` section names + entry prelude, implemented in `protector_matchers.rs` (Commit F). |
| **Denuvo Anti-Tamper** | per-title custom | ✅ (detect + refuse) | ⛔ (deliberate) | ⛔ | Section literally named `.vm` (case-sensitive) detected in `protector.rs` (Commit E); this tool intentionally does not attempt to devirtualize Denuvo — see module doc on `ProtectorFamily::Denuvo`. |
...
| **BattlEye (BEDaisy.sys)** | current | 🟡 | 🟡 (classified as `VmProtect`) | 🟡 | Detected via structural VMP dispatcher fingerprint (Commit I, `protector_signals::scan_rx_sections_for_dispatcher`) OR the literal `.be0` section-name special-case (Commit E) — both route to `ProtectorFamily::VmProtect` since BEDaisy is a scrubbed-section VMP wrapper, not a distinct family. |
| **EasyAntiCheat (EAC)** | ≤2021 driver | 🟡 | 🟡 | 🟡 | Single-VM VMP 2 in driver — the structural dispatcher fingerprint (Commit I) fires on this the same way it does for BattlEye, since both are section-scrubbed stock VMP. |
| EAC modern | proprietary integrity VM | ⛔ | ⛔ | ⛔ | Non-VMP secondary VM. |
| **Riot Vanguard (stub.dll)** | Packman shell + VMP 3 detection routines | 🟡 | 🟡 (classified as `UnknownProtector`) | 🟡 | Two PE sections both named `.stub` special-cased in `protector.rs` (Commit E) to `ProtectorFamily::UnknownProtector` (Packman itself is not VMP); VMP-3 sub-routines inside would classify if unpacked first. |
| Riot Vanguard (vgk.sys) | kernel driver, dispatch-table hooks, PML4 clones | ⛔ | ⛔ | ⛔ | No VM at all; out of scope for a devirtualiser. |
| **Compressor tier** (UPX, MPRESS, Petite, PECompact, Upack) | vanilla + forks | ✅ | — | — | Section-name matchers for all five families implemented in `protector.rs` (Commit E); short-circuits to the specific `ProtectorFamily` variant ("compressed, not virtualised") instead of falling through to `Unknown`. |
...
|---|---|---|---|---|---|
| 1 | P0 | ~~Add class-level "some protector" gate (entropy / W+X / stripped-IAT / EP-outside-`.text` / overlay) with a `RuleScore`-style aggregator~~ **IMPLEMENTED** (Commit E, `42cb238`: `ProtectorFamily` foundation + class-level gate) | Truthful classification on section-renamed VMP, Themida, and unknown packers instead of `Unknown`(0) | M | §2.2 |
| 2 | P0 | ~~Add cheap vendor byte-table + section-name matchers for Themida, Enigma, Obsidium, Code Virtualizer, Armadillo, ASPack, Compressor tier~~ **IMPLEMENTED** (Commit F, `94f1af8`: 9 vendor matchers in `src/protector_matchers.rs`) | 8-10 new families detected with no new abstraction | M | §2.1 |
| 3 | P0 | ~~Add VMP-version string markers (`ZwProtectVirtualMemory` → 3.x, `VBoxRev/VBoxVer` → VMP anti-VM)~~ **IMPLEMENTED** (Commit E, `42cb238`) | Cheaper version discrimination on existing VMP path | S | §2.1 |
| 4 | P0 | ~~Implement the 8 P0 VMP semantic matchers (`Add`, `Ret`, `Ldd`, `Str`, `Vsetvsp`, `PushImm`/`PushReg` split, `Popreg`, `Popf`)~~ **IMPLEMENTED** (Commit G, `505bfed`); extended to the full **35/35** taxonomy in Commit O (`59908c6`, +Commit L `5f42752` for the +14 arithmetic/shift/system matchers in between) | Covers most-frequent handlers in a real virtualised function; unblocks meaningful lift on any VMP sample | L | §3.1 |
| 5 | P1 | ~~Junk-code stripper pass (liveness + peephole) run before `SemanticMatcher::classify`~~ **IMPLEMENTED** (Commit H, `8bd4805`+`ea49498`: pre-pass wired into `handler_classifier`; enhanced with Groups E/F/G — constant folding, dead-store elimination, backward liveness — plus fixed-point iteration in Commit P, `6a5d768`) | Rescues all `is_*_shape` matchers against VMP 3.x mutator envelope; a prerequisite for handler-level lifting on modern builds | L | §5, §6.2 |
...
| 7 | P2 | ~~Register-role canonicaliser (data-flow assignment of VIP/VSP/VKEY/handler-base)~~ **IMPLEMENTED** (Commit K, `92c04af`: pattern-heuristic VIP/VSP/VKEY voter in `src/register_roles.rs`; cross-handler consistency gate added in Commit Q, `0c10fdd`, `src/register_roles_consistency.rs`). Full data-flow assignment (not just pattern-heuristic voting) and the cross-handler DFA needed for VMP 3.7+ merged handlers remain open. | Prerequisite for any lifter emitting IR from VMP 3.x; also stabilises matchers against per-instance register randomisation | XL | §5, §6.2 |
| 8 | P2 | ~~VMP operand-crypto per-version replacement of `crc*31 + val` placeholder — starting with VMP 2.x rolling arithmetic chain (documented)~~ **IMPLEMENTED** (Commit M, `796da51`: `CryptoScheme` enum with `for_version()` dispatch — `None`/`Vmp2Rolling`/`Vmp3PerHandler`/`Placeholder`, `src/decrypt.rs`). Per-handler cryptor op selection for Vmp3 (vs. the current default op set) and cross-check against a real VMP-decrypted operand stream remain open. | Real operand values in devirtualised output instead of placeholder-decrypted noise | L | §4.2 |
| 9 | P2 | 🟡 Partial — real-sample corpus (5-10 VMP-3 hello-world builds, VMProtect Free/Trial) still needed to close AUDIT_REPORT §Days 6-7, which is blocking every "true positive rate" claim. **Real-sample harness implemented** (Commit N, `d320477`: `tests/samples.rs`, gated behind `--features real-samples`, walks `tests/fixtures/<vmpN>/`, checks family/version/dispatch-table against `assert_cmd` runs). **Synthetic-sample harness ALSO implemented** (Commit S, `8932073`) — `src/synthetic_sample.rs` emits VMP-shaped PEs (correct section layout, entry stub, structural dispatcher fingerprint, 256-entry dispatch table, 30 handler shells with byte-perfect matches for 11 semantic categories) that the CLI's own detection stack, register-role voter and semantic classifier must all agree on end-to-end (`tests/synthetic.rs`, 4 e2e tests, gated behind `--features synthetic-samples`). The synthetic path closes a major "can we even verify our own pipeline" gap without waiting for the real corpus — every P0/P1 above is now verifiable against a controlled ground-truth today. The real-sample verification remains open until user-provided binaries land in `tests/fixtures/<vmpN>/`. | Every P0/P1 above becomes verifiable rather than aspirational | M (once samples are available) | §Q2 in AUDIT_REPORT |
