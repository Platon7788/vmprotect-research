...
| 9 | P2 | Real-sample corpus (5-10 VMP-3 hello-world builds, VMProtect Free/Trial) — closes AUDIT_REPORT §Days 6-7 which is blocking every "true positive rate" claim. **Harness implemented** (`tests/samples.rs`, gated behind `--features real-samples`, walks `tests/fixtures/<vmpN>/`, checks family/version/dispatch-table against `assert_cmd` runs) — the corpus itself is still user-provided, so the true-positive-rate claim remains unverified until binaries are actually dropped in per `tests/fixtures/README.md`. | Every P0/P1 above becomes verifiable rather than aspirational | M (once samples are available) | §Q2 in AUDIT_REPORT |
