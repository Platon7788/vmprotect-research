...
| 6 | P1 | Structural VMP dispatcher fingerprint (three-instruction chain `mov r,[VIP]; xor r,key; add r,tbl; jmp [r]` with gap tolerance) alongside section-name lookup | Detects BattlEye BEDaisy, EAC-in-scrubbed-section, renamed-section VMP; converts §4.1 model-mismatch into false-negative-free | L | §2.3, §4.1 |
...
| 9 | P2 | Real-sample corpus (5-10 VMP-3 hello-world builds, VMProtect Free/Trial) — closes AUDIT_REPORT §Days 6-7 which is blocking every "true positive rate" claim | Every P0/P1 above becomes verifiable rather than aspirational | M (once samples are available) | §Q2 in AUDIT_REPORT |
